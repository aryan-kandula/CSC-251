...
static void addItem() {
    ...
    if (qs == null) return;
    try {
        // CHANGE 1: Duplicate name guard — case-insensitive check before inserting
        String trimmed = name.trim();
        boolean exists = inventory.stream()
            .anyMatch(i -> i.getName().equalsIgnoreCase(trimmed));
        if (exists) {
            FarmDialog.message("Duplicate Item",
                "An item named \"" + trimmed + "\" already exists.\n"
                + "Use Restock to add more units, or choose a different name.");
            return;
        }
        inventory.add(new StoreItem(trimmed, Double.parseDouble(ps.trim()), Integer.parseInt(qs.trim())));
        markDirty();
        if (statusBar != null) statusBar.setMessage("Added: " + trimmed);
...
static void scheduleService() {
    ...
    try {
        // CHANGE 3: Validate date format matches MM/DD/YYYY before accepting
        if (!date.trim().matches("\\d{2}/\\d{2}/\\d{4}")) {
            FarmDialog.message("Invalid Date",
...
static String buildReport() {
    ...
    String avgSold = soldCnt == 0 ? "0.00" :
        String.format("%.2f", animals.stream()
    ...
    return String.join("\n",
        ...
        "  Avg listing price     : $" + avgListing,
        "  Avg sold price        : $" + avgSold,
        "",
        "  SERVICES & PAYMENTS",
        "  " + "-".repeat(44),
